...
wss.on('connection', ws => {
  ws.on('message', m => {
    ...
    console.log('Получено сообщение от клиента:', m);

    // Пример: Рассылка полученного сообщения всем клиентам
  ...
  ws.send('Привет, я WebSocket-сервер');

  // Вы не можете отправлять сообщения клиентам здесь сразу после подключения
  // Переместите эту логику в соответствующие части вашего приложения, где происходит регистрация клиента
});
...
app.delete('/api/client/:clientId/product/:productIndex', async (req, res) => {
  ...
  try {
    // Находим клиента по его идентификатору
    ...
    // Сохраняем обновленные данные клиента в базе данных
    await client.save();

    res.status(200).json({ message: 'Продукт успешно удален' });
  } catch (error) {
    console.error('Ошибка при удалении продукта из массива product клиента:', error);
    res.status(500).send('Внутренняя ошибка сервера');
  }
});
...
app.post('/api/updateClientDataPayment', async (req, res) => {
  ...
  try {
    // Добавляем новые данные оплаты клиента в массив payment
    await Client.findOneAndUpdate(
      { _id: requestData._id },
      { $push: { payment: requestData.payment } }
    );
    res.status(200).send('Данные оплаты успешно добавлены');
  } catch (error) {
    console.error(error);
    res.status(500).send('Ошибка при добавлении данных оплаты');
  }
});
...
app.post('/api/searchClients', async (req, res) => {
  ...
  try {
    const lastSixDigits = numberPhone.slice(-6);
    ...
    // Возвращаем результаты поиска клиентов
    res.json(searchData);
  } catch (error) {
